								 ...
								 const std::set<CollisionModel*> *interestList)
{
	...
	int mmodelStartLen = mModels.size(), it1Tracker = -1, it2Tracker;
	for (it1=mModels.begin(); it1!=mModels.end(); it1++) {
		...
		//start from it1 since they are sorted
			if( mmodelStartLen != mModels.size()){
				std::cout <<"Concurrency problem \n";
			}
		for (it2 = it1+1; it2!=mModels.end(); it2++) {
			it2Tracker ++;
			//if it2 is poisoned, something wierd is going on.

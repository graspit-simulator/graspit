...
//
// $Id: robot.cpp,v 1.87 2009/10/08 16:16:53 cmatei Exp $
//
//######################################################################
...
Robot::~Robot()
{
	...
	std::cerr << "Deleted robot: " << name() <<std::endl;
}
...
Robot::loadContactData(QString filename)
{
	...
	if (getName() != robotName) {
		DBGA("Virtual Grasp defined for another robot named: " << robotName << " should be: " << getName().latin1());
		fclose(fp);
		return FAILURE;
	}
	int numContacts;
...
Robot::attachRobot(Robot *r,int chainNum,const transf &offsetTr)
{
  ...
  //this should be changed with fake link
  if(r->getName() == QString("snake")){
	  myWorld->toggleCollisions(false, chainVec[0]->getLink(chainVec[0]->getNumLinks() - 2), r->getBase());
	  myWorld->toggleCollisions(false, chainVec[0]->getLink(chainVec[0]->getNumLinks() - 2), r->getChain(0)->getLink(0));
	  myWorld->toggleCollisions(false, chainVec[0]->getLink(chainVec[0]->getNumLinks() - 2), r->getChain(0)->getLink(1));
  }
...
*/
		//if the joint is almost at its limit, set it to zero
		for(int i = 0; i < numChains; ++i){
			for(int j = 0; j < chainVec[i]->getNumJoints(); ++j){
				if(fabs (chainVec[i]->getJoint(j)->getMax() - chainVec[i]->getJoint(j)->getMin()) > epsilon ) continue;
				for(int l = 0; l < DOFJacobian.rows(); ++l){
					DOFJacobian.elem(l,chainVec[i]->getJoint(j)->getDOFNum()) = 0.0;
				}
			}
		}
/*
		for(int i = 0; i < DOFJacobian.rows(); ++i){
			for(int j = 0; j < DOFJacobian.cols(); ++j){
				std::cout << DOFJacobian.elem(i,j) << " ";
			}
			std::cout << std::endl;
		}
*/
		underDeterminedSolveSVD(DOFJacobian, deltaPR, deltaDOF);
/*
		std::cout << "results: \n";
...
*/
	...
	for(int i = 0; i < numChains; ++i){
		for(int j = 0; j < chainVec[i]->getNumJoints(); ++j){
			...
			if(jnt - chainVec[i]->getJoint(j)->getMax() > epsilon ||
				chainVec[i]->getJoint(j)->getMin() - jnt > epsilon){
					std::cout << "inverse kinematics in invalid joint value: " << i << "th chain, " << j << "th joint" 
						<< jnt << ":" << chainVec[i]->getJoint(j)->getMin() << ":" << chainVec[i]->getJoint(j)->getMax() << "\n";
					return FAILURE;
				}
		}
	}
...
/*Scales the geometry of the body - currently only works along major axes,
skew transformations not yet supported.
*/
SoScale * getScalingNode(SoGroup const * geomRoot){
	SoScale * myScale = NULL;
	for(int childInd = 0; childInd < geomRoot->getNumChildren(); childInd ++){
		if(geomRoot->getChild(childInd)->isOfType(SoScale::getClassTypeId())){
			myScale = static_cast<SoScale *>(geomRoot->getChild(childInd));
		}
	}
		return myScale;
}

/*Scales the geometry and DH parameters of the entire robot*/
void RobotTools::scaleHand(const mat3 & scale, Hand * h){
	RobotTools::scalePalm(scale, h);
	RobotTools::scaleRobot(scale, static_cast<Robot *>(h));
	for(int chainInd = 0; chainInd < h->getNumChains(); chainInd ++)
	{
		h->getWorld()->toggleCollisions(false, h->getChain(chainInd)->getLink(0), h->getPalm());
	}
}

void RobotTools::scalePalm(const mat3 & scale, Hand * h){
	for (int kcInd = 0; kcInd < h->getNumChains(); ++kcInd){
		mat3 currScale = normalizeScale(scale,getScalingNode(h->getPalm()->getIVGeomRoot()));
		transf tPointer = h->getChain(kcInd)->getTran();
		tPointer.set(tPointer.rotation(), currScale * tPointer.translation());
		h->getChain(kcInd)->setTran(tPointer);
	}
	RobotTools::scaleLink(scale, h->getPalm());
	//For every kinematic chain, turn off collisions between first link and palm
	for(int chainInd = 0; chainInd < h->getNumChains(); chainInd ++)
	{
		h->getWorld()->toggleCollisions(false, h->getChain(chainInd)->getLink(0), h->getPalm());
	}
...
void RobotTools::scaleRobot(const mat3 & scale, Robot * rob){
	for(int chainIndex = 0; chainIndex < rob->getNumChains(); chainIndex++){
		RobotTools::scaleChain(scale, rob->getChain(chainIndex));
	}
}


/*Scales the geometry of the joint and DH parameters*/
void RobotTools::scaleChain(const mat3 & scale, KinematicChain * kc){
	for(int linkIndex = 0; linkIndex < kc->getNumLinks(); ++linkIndex){
		RobotTools::scaleJoint(scale, kc->getJoint(linkIndex), kc->getLink(linkIndex));
		RobotTools::scaleLink(scale, kc->getLink(linkIndex));
		//turn off collisions to previous link and following
			if (linkIndex >0)
				kc->getOwner()->getWorld()->toggleCollisions(false, kc->getLink(linkIndex), kc->getLink(linkIndex - 1));
			if (linkIndex < kc->getNumLinks()-1)
				kc->getOwner()->getWorld()->toggleCollisions(false, kc->getLink(linkIndex), kc->getLink(linkIndex + 1));
	}
	return;
}

void RobotTools::scaleJoint(const mat3 & scale, Joint * j, Link * l){
	mat3 currScale = normalizeScale(scale, getScalingNode(l->getIVGeomRoot()));
	j->getDH()->setA(j->getDH()->getA()*currScale[0]);
	j->getDH()->setD(j->getDH()->getD()*currScale[4]);
	j->getTran().toSoTransform(j->getIVTran());
	return;
}

void RobotTools::scaleLink(const mat3 & scale, Link * l){
	//scale the robot geometry
	RobotTools::scaleBody(scale, static_cast<Body *>(l));
	//scale the DH parameters associated with this body
}


void RobotTools::scaleGraspableBody(const mat3 & scale, GraspableBody * b){
	//b->getWorld()->destroyElement(b,false);
	b->getWorld()->getCollisionInterface()->removeBody(b);
	SoGroup * geomRoot = b->getIVGeomRoot();
	SoScale * myScale;
	myScale = getScalingNode(geomRoot);
	//Normalize scale to current transformation, then adjust center of gravity
	b->setCoG(RobotTools::normalizeScale(scale,myScale)* b->getCoG());
	if(!myScale){
		myScale = new SoScale;
		geomRoot->insertChild(myScale,0);
		myScale->scaleFactor.setValue(1.0, 1.0, 1.0);
	}
	myScale->scaleFactor.setValue(scale[0], scale[4], scale[8]);
	//set the dynamic properties. This need
	//static_cast<GraspableBody *>(b)->setDefaultDynamicParameters();
//	b->setCoG(scale *b->getCoG());
	//FIXME:This is the I matrix from the flask, not in any way correct
	double I[] = {4853.0, -1.1196, -6.5156, -1.1196, 4853.0, 47.542, -6.5156, 0.0, 2357.6};
	b->setInertiaMatrix(I);
//	b->setCoG(RobotTools::normalizeScale(scale,getScalingNode(geomRoot)) * b->getCoG());
	b->setMaxRadius(b->computeDefaultMaxRadius());
	b->addToIvc();
}

void RobotTools::scaleBody(const mat3 & scale, Body * b){
	//b->getWorld()->destroyElement(b,false);
	b->getWorld()->getCollisionInterface()->removeBody(b);
	SoGroup * geomRoot = b->getIVGeomRoot();
	SoScale * myScale;
...
/*Return a scaling matrix that combines the inverse of the last scale with the current scale*/
mat3 RobotTools::normalizeScale(const mat3 & scale, SoScale * myScale){
	mat3 oldScale = mat3::IDENTITY;
	if(myScale){
		//a scaling exists, invert it and multiply it by the current scaling to normalize it.
		oldScale[0] = 1/myScale->scaleFactor.getValue()[0];
		oldScale[4] = 1/myScale->scaleFactor.getValue()[1];
		oldScale[8] = 1/myScale->scaleFactor.getValue()[2];
	}
		return oldScale * scale;
}

void RobotTools::setHandMaterial(Hand * h, materialT mat){
	h->getPalm()->setMaterial(mat);
	setRobotMaterial(h,mat);
}

void RobotTools::setRobotMaterial(Robot * r, materialT mat){
		for(int kind = 0; kind < r->getNumChains(); kind++)
			for(int lind = 0; lind < r->getChain(kind)->getNumLinks(); lind++)
				r->getChain(kind)->getLink(lind)->setMaterial(mat);

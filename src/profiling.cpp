...
//
// $Id: profiling.cpp,v 1.7 2009/12/01 18:45:42 cmatei Exp $
//
//######################################################################
...
ProfileInstance::getTotalTimeMicroseconds()
{
	...
	/*
	if(result > 1.0e10) {
	  if (mRunning) std::cerr << "Timer is running\n";
	  std::cerr << " Current time: " << currentTime << "\n";
	  std::cerr << "Starting time: " << mStartTime << "\n";
	  std::cerr << "   Diff (raw): " << currentTime - mStartTime << "\n";
	  std::cerr << "   Total time: " << totalTime << "\n";	  
	  std::cerr << "Result (conv): " << result << "\n";
	}
	*/
	return result;
}

...
//
// $Id: world.cpp,v 1.86 2009/08/14 18:11:05 cmatei Exp $
//
//######################################################################
...
World::World(QObject *parent, const char *name, IVmgr *mgr) : QObject(parent,name)
{
	myIVmgr = mgr;
	currentControl = NULL;
	numBodies = numGB = numRobots = numHands = numSensors = 0;
...
World::loadFromXml(const TiXmlElement* root,QString rootPath)
{
	...
	while(child!=NULL){
		...
		if (elementType == "obstacle") {
			...
			if(!graspItGUI->useConsole() && myIVmgr)
	...
	if (!cameraFound  && !graspItGUI->useConsole() && myIVmgr) {

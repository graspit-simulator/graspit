...
//
// $Id: ivmgr.cpp,v 1.64 2009/08/14 18:11:05 cmatei Exp $
//
//######################################################################
  ...
  QWidget(parent,name,f)
{
  ...
  //myViewer->setBackgroundColor(SbColor(.9,.9,.9));
...
IVmgr::emptyWorld()
{
	if(selectionRoot)//in useconsole mode, no selectionRoot is created.
	{
		selectionRoot->deselectAll();
 ...
 // jointDraggerSep->ref();
  ...
  for(d=0;d<robot->getNumDOF();d++) {
    if ((j=activeDOFs[d]) != -1) {
	  ...
	  SoTransform * tr = chain->getJoint(j)->getIVTran();
//	  tr->translation.set = chain->getLink(j)->getTran()
      jointDraggerSep->addChild(tr);
      draggerInfoList.push_back(dInfo);
    }
  }

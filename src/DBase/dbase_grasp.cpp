...
//
// $Id: dbase_grasp.cpp,v 1.25 2009/10/08 16:13:11 cmatei Exp $
//
//######################################################################
/*! \file 
...
{
	if (mPlanner) {delete mPlanner; mPlanner = NULL;}
	if (mTimerSensor) delete mTimerSensor;
	DBGAF(mLogStream,"Planner successfully deleted");
	mLogStream.close();
}

bool DBaseBatchPlanner::processArguments(int argc, char **argv, GraspableBody * gb, Hand * h)
{
	if (argc < 8) {
	...
	if (mHand->getNumVirtualContacts()==0) {
		...
		DBGAF(mLogStream,"Hint: make sure capitalization in robot name is correct (i.e. HumanHand20DOF, Barrett)");
		getchar();
		return false;
	}
...
bool DBaseBatchPlanner::startPlanner()
{
	...
	if (mType == DEXTEROUS) {
		...
		if(!mPlanner)
			mPlanner = new GuidedPlanner(mHand);//GuidedPlanner(mHand);
		int numChildren;
...
//		((GuidedPlanner*)mPlanner)->setMaxChildren(numChildren);
		((GuidedPlanner*)mPlanner)->setMaxChildren(1);
...
bool DBaseBatchPlanner::isActive(){return mPlanner->isActive();}

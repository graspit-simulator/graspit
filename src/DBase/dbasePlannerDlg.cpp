...
#include "DBPlanner/off_wrapping_aligner.h"
#include "debug.h"
...
void DBasePlannerDlg::rankGraspsButton_clicked(){
	...
	if(mAligner)
		delete mAligner;
	mAligner = new db_planner::OffWrappingAligner(db_planner::CachingAligner(*mDBMgr, false, false, alignmentMethodComboBox->currentText().toStdString()));
...
void DBasePlannerDlg::show3DObject(bool isNbr){
	...
	if(isNbr){
		...
		std::cout << "Grasp ID: " << mOriginalGrasps[mCurrentOriginalGrasp]->GraspId() << std::endl;
	}

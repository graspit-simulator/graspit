...
//
// $Id: body.cpp,v 1.76 2009/10/08 16:15:07 cmatei Exp $
//
//######################################################################
...
double DynamicBody::defaultMass = 0.1;
...
Body::loadFromXml(const TiXmlElement *root, QString rootPath)
{
	...
	if (!element) {
		...
		//Scaling factor -- Warning, order is important here as loading IV files will clear the GeomRoot, so this must be done after geometry is loaded
	element = findXmlElement(root,"scale");
	if(element){
			SoScale* scale = new SoScale();
			scale->scaleFactor.setValue(QString(element->GetText()).toDouble(),QString(element->GetText()).toDouble(), QString(element->GetText()).toDouble());
			getIVGeomRoot()->insertChild(scale,0);
	}
	return SUCCESS;
}
...
Body::load(const QString &filename)
{
	...
	//the root path is the directory in which the file is placed
	...
	if (fileType != "xml") {
		...
		QString relFilename = relativePath(filename, root);
...
Body::loadGeometryOFF(const QString& filename) {
  ...
  for (long vertex = 0; vertex < num_vertices; ++vertex) {
    ...
    if (line.fail()) return OFFReadFailure();
	  ...
	  mRawCoordinates.push_back(position(x,y,z));
  }
...
Body::addIVMat(bool clone)
{
	...
	IVMat->diffuseColor = SbColor(1.0,0.0,0.0);
...
/*!
	Add this current contact force to body's total contact forces.
*/
void Body::addContactForce(double *f, Contact * c){
	  for(int ind = 0; ind < 6; ind ++)
		  contactForceSum[ind] += f[ind];
}
...
std::vector<position>
Body::getRawGeometryVertices() const
{
	return mRawCoordinates;
}

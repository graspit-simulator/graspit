...
SimAnn::variableNeighbor(VariableSet *set, double T, VariableSet *target)
{
	...
	for (int i=0; i<set->getNumVariables(); i++) {
		...
		v = var->mMaxVal + var->mMaxJump;//1.0; //start off ilegal
		int loop = 0;
		while ( v>var->mMaxVal || v < var->mMinVal ) {
			...
			//check if we need to go back when the variable is circular
			if (var->isCircular()) {
			...
			//check if the neighbor goes beyond the possible range for the variable
...
#if 0
	//create a normal distribution with mean(in), std(0.5*mean)

	//return a random value according to this distribution
	boost::mt19937 rng; // I don't seed it on purpouse (it's not relevant)
	boost::normal_distribution<> nd(in, (0.2*fabs(in)+0.0000001)/(conf+0.0000001));

...
//
// $Id: egPlanner.cpp,v 1.39 2009/05/07 19:57:26 cmatei Exp $
//
//######################################################################
...
EGPlanner::checkTerminationConditions()
{
	...
	if (mMaxSteps != -1 && mCurrentStep > mMaxSteps){ 
		if (!mRepeat) {
			std::cerr << "pause planner due to mRepeat(false), max: " << mMaxSteps << ", current: " << mCurrentStep << std::endl;
			pausePlanner();
			termination = true;
		} else {
			resetParameters();
		...
		//std::cerr << "check the time: " << getRunningTime() << " compared with max time " << mMaxTime << std::endl;
		if(getRunningTime() - mLastTime > 30)
		{
			std::cerr << "running time: " << getRunningTime() << std::endl;
			mLastTime = getRunningTime();
		}
		if (getRunningTime() > mMaxTime) {
			std::cerr << "stepping out since max time is hit: " << "running time is: " << getRunningTime() << std::endl;
			stopPlanner();
			termination = true;
		}
	}
	if (termination) {
		std::cerr << "termination(true): emitting complete() and exiting" << std::endl;
		emit complete();
	}
	return termination;
}
...
{
	//std::cout << (double)clock() << ", " << mStartTime << std::endl;
	if (isActive()) return mRunningTime + (clock() - mStartTime) / CLOCKS_PER_SEC;
...
EGPlanner::pausePlanner()
{
	...
	if (!mMultiThread)
	{
		std::cerr << " planner to be paused, not multithreaded now, running time: " << mRunningTime << ", start time: " << mStartTime <<  ", emitting complete()" << std::endl;
		emit complete();
	}
}

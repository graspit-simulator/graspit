...
//
// $Id: searchState.cpp,v 1.39 2009/07/27 15:54:34 cmatei Exp $
//
//######################################################################
...
PositionState* PositionState::createInstance(StateType type, const Hand *h)
{
	switch(type) {
		...
		case SPACE_LOCAL:
			return new PositionStateLocal(h);
...
bool HandObjectState::readFromFile(FILE *fp)
{
	...
	{
		std::cout << "statetype should be POSE_DOF or POSE_EIGEN" << std::endl;
		return false;
	}
	...
	if ( (StateType)type != SPACE_COMPLETE && (StateType)type != SPACE_APPROACH &&
		 (StateType)type != SPACE_AXIS_ANGLE && (StateType)type != SPACE_ELLIPSOID && (StateType)type != SPACE_LOCAL)
	{
		std::cout << "statetype is not valid" << std::endl;
		return false;
	}
...
HandObjectState::hideVisualMarker()
{
	...
	if ( i < 0 ) {
		DBGA("Could not find search state marker in the world root");
	} else {
...
{
	//WRONG::if (s1->getEnergy() <= s2->getEnergy()) return true; 
...
{
	//WRONG::if (s1->getDistance() <= s2->getDistance()) return true; 

...
GuidedPlanner::startChild(const GraspPlanningState *seed)
{
	...
	if (mChildThreads) {
		oldThread = mHand->getWorld()->thread();
		mHand->getWorld()->moveToThread(child);
		child->startThread();
...
#else
	//this is special for SEMANTIC PLANNER
	//we want to set a high belief value
	child->getTargetState()->getPosition()->setAllConfidences(0.95);
	if(child->getTargetState()->getPosition()->getVariable("dist"))
		child->getTargetState()->getPosition()->getVariable("dist")->setConfidence(0.5);
	child->getTargetState()->getPosition()->setAllFixed(true);
	child->getTargetState()->getPosture()->setAllConfidences(0.95);
	child->getTargetState()->getPosture()->setAllFixed(true);
#endif	
...
GuidedPlanner::stopChild(SimAnnPlanner *pl)
{
	...
	if (j) {
		...
		if (s2->getEnergy() < 10.0) {
			//If this is a good grasp, mark it as red
			s2->setIVMarkerColor(1,0,0);
		} else {
			//If this is not a good grasp, mark it as grey
...
GuidedPlanner::mainLoop()
{
	...
	//this is added for using input target solution reference with a certain confidence
	GraspPlanningState *input = NULL;
	if ( processInput() ) {
		input = mTargetState;
	}
	// call main simann iteration
	SimAnn::Result r = mSimAnn->iterate(mCurrentState, mEnergyCalculator, input, NULL);
#else
	SimAnn::Result r = mSimAnn->iterate(mCurrentState, mEnergyCalculator, NULL, NULL);
#endif
	...
	//std::cout << "energy: " << mCurrentState->getEnergy() << std::endl;
	if (r==SimAnn::JUMP && mCurrentState->getEnergy() < bestEnergy) {
		GraspPlanningState *insertState = new GraspPlanningState(mCurrentState);
		//if(mCurrentState->getVariable("EG 0")->getConfidence() > 0.1)
			//std::cout << "guidedPlanner::copying: EG0 conf: " << mCurrentState->getVariable("EG 0")->getConfidence() << " EG1 conf: " << mCurrentState->getVariable("EG 1")->getConfidence() << std::endl;

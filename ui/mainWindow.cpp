...
#endif
#ifdef DOF_CTRL_ENABLED
#include "dofControlDlg.h"
#endif
#include "taskControlDlg.h"
#ifdef BLIND_PLANNER_ENABLED
#include "BlindPlanner/blindPlannerDlg.h"
#endif
#ifdef UNCERTAINTY_PLANNER_ENABLED
#include "UncertaintyPlanner/uncertaintyPlannerDlg.h"
#endif
#ifdef STRATEGY_PLANNER_ENABLED
#include "StrategyPlanner/strategyPlannerDlg.h"
#endif
...
MainWindow::MainWindow(QWidget *parent) 
{
	...
	QObject::connect(mUI->dofControl, SIGNAL(triggered()), 
					 this, SLOT(misDofControlDlg()));
	QObject::connect(mUI->taskControl, SIGNAL(triggered()), 
					 this, SLOT(misTaskControlDlg()));
	QObject::connect(mUI->blindPlanner, SIGNAL(triggered()),
					 this, SLOT(misBlindPlannerDlg()));
	QObject::connect(mUI->uncertaintyPlanner, SIGNAL(triggered()),
							 this, SLOT(misUncertaintyPlannerDlg()));
	QObject::connect(mUI->strategyPlanner, SIGNAL(triggered()),
							 this, SLOT(misStrategyPlannerDlg()));
...
void MainWindow::setMainWorld( World *w )
{
  ...
  if (world != NULL && world->getCurrentHand() !=NULL) {
      DBGA("Launching dbase + blind planner dlgs");
      dbaseGUIAction_activated();
      misBlindPlannerDlg();
  } else {
      DBGA("Load graspit with a hand to make things easier...");
  }
...
void MainWindow::misDofControlDlg()
{
#ifdef DOF_CTRL_ENABLED
	DOFControlDlg *dlg = new DOFControlDlg(mWindow, graspItGUI->getIVmgr()->getWorld());
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();
#endif
...
void MainWindow::misTaskControlDlg()
{
	TaskControlDlg *dlg = new TaskControlDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();
}

void MainWindow::misBlindPlannerDlg()
{
#ifdef BLIND_PLANNER_ENABLED
	BlindPlannerDlg *dlg = new BlindPlannerDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();
#endif
...
void MainWindow::misUncertaintyPlannerDlg()
{
#ifdef UNCERTAINTY_PLANNER_ENABLED
	UncertaintyPlannerDlg *dlg = new UncertaintyPlannerDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();
#endif
...
void MainWindow::misStrategyPlannerDlg()
{
#ifdef STRATEGY_PLANNER_ENABLED
	StrategyPlannerDlg *dlg = new StrategyPlannerDlg(mWindow);
	dlg->setAttribute(Qt::WA_ShowModal, false);
	dlg->setAttribute(Qt::WA_DeleteOnClose, true);
	dlg->show();
#endif
...
void MainWindow::startTapTap(){
//	world->currentControl = new TapTapController(world, true);
}

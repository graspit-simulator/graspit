...
{ 
	...
	energyBox->insertItem("Number of Contacts");
	...
	plannerTypeBox->insertItem("Num-Contacts");
	...
	spaceSearchBox->insertItem("Local");
...
void EigenGraspPlannerDlg::setMembers( Hand *h, GraspableBody *b )
{
	...
	mHandObjectState->getPosition()->setTran(mHand->getTran()*mObject->getTran().inverse());
...
{
	/*
	setRefTran will set the current hand pose as the origin for perturbation
	after the refTran is set, all the pose change is considered with respect
	to the frame defined by the refTran
	*/
	...
	/*
	For approach and local styles, we search w.r.t the hand's current coordinate frame.
	Thus, we set the current hand's tran as the refTran.
	Caution, 
	*/
	else if ( s==QString("Approach") ) {
		mHandObjectState->setPositionType(SPACE_APPROACH);
		mHandObjectState->setRefTran( mHand->getTran() );
	} else if ( s==QString("Local") ) {
		mHandObjectState->setPositionType(SPACE_LOCAL);
		mHandObjectState->setRefTran( mHand->getTran() );
	} else {
		fprintf(stderr,"WRONG SEARCH TYPE IN DROP BOX!\n");
	}
	mHandObjectState->reset();
	if( s==QString("Axis-angle") || s==QString("Ellipsoid") || s==QString("Complete") )
		mHandObjectState->getPosition()->setTran(mHand->getTran()*mObject->getTran().inverse());//handInObj = handInWorld * worldInObj
	else
		mHandObjectState->getPosition()->setTran(transf::IDENTITY);
	updateVariableLayout();
	//force a reset of the planner
...
void EigenGraspPlannerDlg::updateResults(bool render)
{
	...
	*/
	...
	if (render) {
		...
		dofVals.resize(mHand->getWorld()->getRobot(1)->getNumDOF());
		mHand->getWorld()->getRobot(1)->invKinematics(mHand->getBase()->getTran(), &dofVals[0], 0);
		mHand->getWorld()->getRobot(1)->forceDOFVals(&dofVals[0]);
		*/
	}
}
...
void EigenGraspPlannerDlg::readPlannerSettings()
{
	...
	} else if ( s == QString("Number of Contacts") ) {
		mPlanner->setEnergyType(NUM_CONTACTS);
	} else {
	...
	GraspPlanningState* ts = mPlanner->getTargetState();
	assert(ts);
	//parameter confidence
	bool hasTarget = false;
	for (int i=0; i<mHandObjectState->getNumVariables(); i++) {
		if (varInput[i]->isChecked()) {
			ts->getVariable(i)->setFixed(true);
			ts->getVariable(i)->setConfidence(0.95);
			hasTarget = true;
		}
		else
		{
			ts->getVariable(i)->setFixed(false);
		}
	}
}

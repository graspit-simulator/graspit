...
//
// $Id: graspCaptureDlg.cpp,v 1.14 2009/07/02 21:28:14 cmatei Exp $
//
//######################################################################
...
GraspCaptureDlg::saveToDBaseButtonClicked()
{
	...
	for (it=mGrasps.begin(); it!=mGrasps.end(); it++) {
		...
		tempArray[1] *= angleLineEdit->text().toDouble();
		tempArray[2] *= angleLineEdit->text().toDouble();
		tempArray[3] *= angleLineEdit->text().toDouble();
		...
		tempArray[0] += xLineEdit->text().toDouble();
		tempArray[1] += yLineEdit->text().toDouble();
		tempArray[2] += zLineEdit->text().toDouble();

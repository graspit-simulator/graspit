...
//! Allows the hand to move locally
/*! smapling orientation around the approaching direction
	and the location in x,y,z dimension locally
	*/
class PositionStateLocal : public PositionState
{
protected:
	void createVariables();
public:
	PositionStateLocal(const Hand *h) : PositionState(h){createVariables();}
	StateType getType() const {return SPACE_LOCAL;}
	transf getCoreTran() const;
	void setTran(const transf &t);
};

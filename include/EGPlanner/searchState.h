...
private:
	...
	int mIndex;
	//! Used to fix the base of the hand, work with snake robot
	bool mBaseFixed;
...
public:
	GraspPlanningState(Hand* hand) : HandObjectState(hand) {mBaseFixed = false;}
	...
	bool getBaseFixed()const {return mBaseFixed;}
	void setBaseFixed(bool b){mBaseFixed = b;}
...
void VariableSet::copyValuesFrom(const VariableSet *s)
{
	for (int i=0; i<(int)s->mVariables.size(); i++) {
		...
		mVariables[i]->setConfidence( s->mVariables[i]->getConfidence() );
	}

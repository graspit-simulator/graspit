...
enum JointT {REVOLUTE,PRISMATIC,SNAKE};
...
public:
  ...
  virtual transf getTran(double jointVal) const {
    return DH->getTran(jointVal + c,DH->getD());
  }
  virtual transf getDynamicsTran() const {
...
/*
This class is specially designed for snake robot
*/
class SnakeJoint : public RevoluteJoint {
	int numSnakeLinks;
	double lengthSnake;
	bool master;
	double dofVal;
	...
	int initMaster(const TiXmlElement* root, int jnum);
	int initAux(const TiXmlElement* root, int jnum);

public:
	SnakeJoint (KinematicChain *k, bool m) : RevoluteJoint(k), master(m), dofVal(0){}
	virtual int initJointFromXml(const TiXmlElement* root, int jnum);
	virtual int setVal(double q);
	...
	virtual transf getDynamicsTran() const;
	/*! Returns the current value of this joint. 
		this is re-engineered to return the dof, not the joint.
		*/
	virtual double getVal() const;// const {return DH->getTheta() - c;}
};

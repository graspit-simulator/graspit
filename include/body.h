...
//
// $Id: body.h,v 1.44 2009/09/29 22:24:52 cmatei Exp $
//
//######################################################################
...
protected:
  ...
  //! records the raw coordinates from the geometry file
  std::vector<position> mRawCoordinates;
...
public:
 ...
 virtual int saveToXml(QTextStream& xml);
  ...
  virtual void addIVMat(bool clone = false);
  ...
  //! Where on the body the Flock of Birds sensor is mountde
  ...
  */
  ...
  void addContactForce(double *f, Contact * c = NULL);
  double * getDynamicForce(){return contactForceSum;};
  ...
  std::vector<position> getRawGeometryVertices() const;
};
...
public:
  ...
  */
   ...
   */
  virtual void resetDynamicsFlag() {
...
class Link : public DynamicBody {
 ...
 static LinkT getType(){return LINK;};
